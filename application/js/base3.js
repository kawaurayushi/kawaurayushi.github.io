...
function OddEven(str){
  ...
  $( "#oe").html('<a data-toggle="modal" href="#oeven">Odd/Even</a>');
    modal+="<span><code>Odd / Even</code>";
    modal+='<pre >';
    modal+='odd  : ' + escapeHTML(a) + "\n";
    modal+='even : ' + escapeHTML(b) + "\n";
    modal+="</pre>";
    modal+="</span>";
  $( "#OEBody").html(modal);
...
function tel1(str){
	...
	var c="";
	while( i<= str.length){
		if( (c.charAt(0) == str.charAt(i)) || c.length <1 ){
			c += str.charAt(i);
		}else{
...
function UpdateModal(str){
	...
	$("#tel1").html('<a data-toggle="modal" href="#tel1m">TelPad1</a>');
	$("#tel1word").html("original : " + Hex2String(str));
	$("#tel1pad").html(tel1( Hex2String(str) ));

	$("#tel2").html('<a data-toggle="modal" href="#tel2m">TelPad2</a>');
...
$(document).ready(function() {
  var key1=0,key2=0,key3=0,key4=0,key5=0,key6=0,key7=0;
...
//       KeyMap[ String(json.keywords[index].word).trim() ] = String(json.keywords[index].atbash).trim() ;
  ...
  function dolookup() {
    ...
    if($('#vauto').is(':checked')){
       ...
       $('#vres').html(vigeneredecrypt(orig,$('#vkey').val()));
    }
 }
});
